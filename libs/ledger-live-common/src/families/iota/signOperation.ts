    ...
    return new Observable((o) => {
        void (async function () {
            try {
                ...
                const transactionPayload = await buildTransactionPayload(account, transport, inputs, outputs, indexation);
                ...
                for (const output of outputs) {
                    ...
                    if (!output.isDustAllowance && (output.address != account.freshAddress || outputs.length == 0)) {
                        value += output.amount;
