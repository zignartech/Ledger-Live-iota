...
const currency = getCryptoCurrencyById("iota");
...
const iota: AppSpec<IotaTransaction> = {
  name: "Iota",
  currency,
  appQuery: {
  ...
  mutations: [
    {
      ...
      test: ({ account, accountBeforeTransaction, operation }) => {
        const rewards =
          (accountBeforeTransaction as IotaAccount).iotaResources
            ?.rewards || 0;
    ...
    },
    {
      ...
      transaction: ({ account, siblings, bridge, maxSpendable }) => {
        ...
        const amount = subAccount.balance
          .div(1.9 + 0.2 * Math.random())
          .integerValue();
        const updates: Array<Partial<IotaTransaction>> = [
          {
            mode,
    ...
    },
    {
      ...
      maxRun: 1,
      transaction: ({ account, bridge, maxSpendable }) => {
        const rewards = (account as IotaAccount).iotaResources?.rewards;
      ...
      test: ({ account }) => {
        botTest("algoResources rewards is zero", () =>
          expect(
            (account as IotaAccount).iotaResources &&
              (account as IotaAccount).iotaResources.rewards.eq(0)
          ).toBe(true)
        );
      },
    },
