...
import type { IotaAccount, IotaResources, Transaction } from "./types";
import { extractTokenId } from "./tokens";
import {
  IOTA_MAX_MEMO_SIZE,
  recipientHasAsset,
  isAmountValid,
  computeIotaMaxSpendable,
} from "./logic";
...
export const getTransactionStatus = async (
  ...
  invariant(a.iotaResources, "Iota family expected");
  const iotaResources = a.iotaResources as IotaResources;

  const algoSpendableBalance = computeIotaMaxSpendable({
  ...
  switch (t.mode) {
    ...
    case "claimReward": {
      ...
      if (estimatedFees.gt(iotaResources.rewards)) {
  ...
  if (t.memo && t.memo.length > IOTA_MAX_MEMO_SIZE) {
