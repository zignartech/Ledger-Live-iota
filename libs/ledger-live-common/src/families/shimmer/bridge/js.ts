...
const shimmerUnit = getCryptoCurrencyById("shimmer").units[0];
...
const txToOps =
  ...
  (tx: Record<string, any>): Operation[] => {
    ...
    const value = parseCurrencyUnit(shimmerUnit, tx.amount);
...
const getAccountShape = async (info) => {
  ...
  const balance = balanceMatch
    ? parseCurrencyUnit(shimmerUnit, String(balanceMatch.amount))
    : new BigNumber(0);
...
const createTransaction = (a: Account): Transaction => {
  throw new CurrencyNotSupported("shimmer currency not supported", {
...
const getTransactionStatus = (a: Account): Promise<TransactionStatus> =>
  Promise.reject(
    new CurrencyNotSupported("shimmer currency not supported", {
      currencyName: a.currency.name,
    })
  );
